package com.twimba.fin.processing;
...
public class StatementExecutor {
	...
	private void executeStatements(){
		...
		statements.add("select symbol, avg(ask) from YahooQuoteEvent.win:time(30 sec)");
		statements.add("select evt.symbol, evt.change from pattern [" +
					   "every evt=YahooQuoteEvent(change>1) where timer:within(120 seconds)" +
					   "] group by evt.symbol");
